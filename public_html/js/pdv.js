...
function renderizarKanban(pedidos) {
    ...
    pedidos.forEach(p => {
        if (!contadores.hasOwnProperty(p.status)) return;
        ...
        let proximoStatus = '';
        let textoBotao = '';
        if (p.status === 'PENDENTE') { proximoStatus = 'PREPARANDO'; textoBotao = '▶️ Iniciar Preparo'; }
        else if (p.status === 'PREPARANDO') { proximoStatus = 'A_CAMINHO'; textoBotao = '🚀 Enviar / Pronto'; }
        else if (p.status === 'A_CAMINHO') { proximoStatus = 'CONCLUIDO'; textoBotao = '✅ Concluir'; }
