...
async function carregarProdutos() {
    if (!_supabase) {
        ...
        produtosList = [
            {id: 1, categoria_id: 1, nome: 'Bolo de Cenoura', descricao: 'Com cobertura generosa de brigadeiro', preco: 15.00, estoque: 5, imagem_url: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=300'},
            {id: 2, categoria_id: 2, nome: 'Brigadeiro Tradicional', descricao: '20g de puro chocolate belga', preco: 4.50, estoque: 10, imagem_url: 'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=300'}
        ];
...
async function finalizarPedido(event) {
    ...
    // 1. Validação Final de Estoque antes de tentar enviar
    for (const item of carrinho) {
        const prod = produtosList.find(p => p.id === item.id);
        if (!prod || item.qtd > prod.estoque) {
            alert(`O produto "${item.nome}" não possui estoque suficiente. Estoque disponível: ${prod ? prod.estoque : 0}.`);
            return; // Aborta a execução do envio
        }
    }
    ...
    try {
        if (!_supabase)
            throw new Error('Configure o Supabase no app.js!');

        // Cadastra/Atualiza o Cliente
        ...
        // 2. Atualiza o estoque no Supabase e na memória local
        for (const item of carrinho) {
            const prod = produtosList.find(p => p.id === item.id);
            if (prod) {
                const novoEstoque = prod.estoque - item.qtd;
                prod.estoque = novoEstoque; // Atualização na memória local

                // Atualiza o estoque na tabela 'produtos' do Supabase
                await _supabase
                    .from('produtos')
        ...
        // 3. Atualiza a lista na tela imediatamente
        renderizarProdutos(produtosList);
